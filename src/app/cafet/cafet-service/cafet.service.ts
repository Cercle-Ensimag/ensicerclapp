...
export class CafetService {
	...
	getUsers(): Observable<CafetUser[]> {
		if (!this._users){
			this._users = this.db.list<CafetUser>(
				'cafet/users'
			).valueChanges().pipe(
	...
	getUser(emailId: string): Observable<CafetUser> {
		if (!this._user[emailId]){
			this._user[emailId] = this.db.object<CafetUser>(
				"cafet/users/" + emailId
			).valueChanges().pipe(
				shareReplay(1)
			);
	...
	getArchivesUsers(): Observable<CafetUser[]> {
		if (!this._archivesUsers){
			this._archivesUsers = this.db.list<CafetUser>(
				'cafet/archives/users'
			).valueChanges().pipe(
				shareReplay(1)
			);
		}
	...
	getArchivesUser(emailId: string): Observable<CafetUser> {
		if (!this._archivesUser[emailId]){
			this._archivesUser[emailId] = this.db.object<CafetUser>(
				"cafet/users/" + emailId
			).valueChanges().pipe(
				shareReplay(1)
			);
	...
	getTrezAccounts(): Observable<CafetUser[]> {
		if (!this._trezAccounts){
			this._trezAccounts = this.db.list<CafetUser>(
				'cafet/trezo/accounts'
			).valueChanges().pipe(
				shareReplay(1)
			);
		}
	...
	getCafetResps() {
		if (!this._cafetResps){
			this._cafetResps = this.db.list<CafetResp>(
				'cafet/cafetResps/resps'
			).valueChanges().pipe(
				shareReplay(1)
			);
		}
	...
	getUserEmailId(email: string, exte: boolean) {
		if (exte) {
			return "%exte%" + Tools.getEmailIdFromEmail(email);
		} else {
			return Tools.getEmailIdFromEmail(email);
		}
	}
	...
	newTransaction(user: CafetUser, value: number) {
		...
		let newCredit = Tools.round(oldCredit + value, 2);
	...
	newDayTransaction(user: CafetUser, value: number) {
		...
		return this.auth.getEmailId().pipe(
			...
			emailId => this.db.object<any>(
				'cafet/cafetResps/dayTransactions/' + user.emailId + "/" + date
			).set({
				value: value,
				date: date,
				resp: emailId
	...
	getDayTransactions() {
		if (!this._dayTransactions) {
			this._dayTransactions = this.db.object<any>(
				'cafet/cafetResps/dayTransactions'
			).valueChanges().pipe(
				map(dt => dt ? dt : {}),
				shareReplay(1)
			);
	...
	deleteDayTransaction(emailId: string, transId: string) {
		return this.db.object<any>(
			"cafet/cafetResps/dayTransactions/" + emailId + "/" + transId
		).remove();
	}
	...
	getUserName(user: CafetUser): string {
		if (!user.profile) {
			return Tools.getNameFromEmailId(user.emailId.replace(/^%[a-z]+%/, ''));
		} else {
			let exte = user.profile.exte ? " *": '';
			return Tools.titleCase(user.profile.firstName + " " + user.profile.lastName) + exte;
		}
	}

	getHistory(emailId: string): Observable<Transaction[]> {
		if (!this._history[emailId]) {
			this._history[emailId] = this.db.list<Transaction>(
				"cafet/history/"+emailId, ref => ref.orderByChild('date')
			).valueChanges().pipe(
